...
def unzipper_chungus_deluxe(max_segment_length):
    ...
    for segment_length in range(max_segment_length):
        zip_file = glob.glob(f"{zips_path}trajectories_t{TRAJECTORIES}_r{RULES}_s{segment_length}.zip")
        print(f"{segment_length} files:", zip_file)
        ...
        aggregate_trained_satisfaction_segments[segment_length] = trained_satisfaction_segments

        shutil.rmtree("temp_trajectories")

    trueRF_path = f"trueRF_trajectories/trueRF_{num_trajectories}_trajectories_3_rules.pkl"
...
def get_trained_generation_averages_and_best_generation(
    ...
    for segment_length, generation_segments in aggregate_trained_satisfaction_segments.items():
        ...
        best_gen_index = np.argmax(aggregate_trainedRF_average_satisfaction_segments[segment_length])
        aggregate_trainedRF_best_generation[segment_length] = aggregate_trainedRF_average_satisfaction_segments[segment_length][best_gen_index]
...
def graph_normalized_segments_over_generations(
    ...
    for segment_length in aggregate_trainedRF_average_satisfaction_segments.keys():
        aggregate_trainedRF_average_satisfaction_segments[segment_length] = [
            avg_segments / max_avg_trueRF_segments
            for avg_segments in aggregate_trainedRF_average_satisfaction_segments[segment_length]
        ]

    plt.figure()
    plt.plot(
        x_values, trueRF_average_satisfaction_segments, label="Ground Truth"
    )
...
def graph_gap_over_pairs(trueRF_best_generation, aggregate_trainedRF_best_generation):
    x_values = sorted(aggregate_trainedRF_best_generation.keys())
    y_values = []

    true_best_avg = np.mean(trueRF_best_generation)
    for segment_length in x_values:
        trained_best_avg = np.mean(aggregate_trainedRF_best_generation[segment_length])
