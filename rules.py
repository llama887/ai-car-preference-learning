...
RULE_ORDER = [1, 2, 3]   # RULE_ORDER[i] = ith rule to be checked
# (!! rule "ORDER", it's not the order rules are checked, but rather indicates which rules are included !!)
PARTIAL_REWARD = False

def to_segment(segment):
    return [StateActionPair(radars=segment[0:5].copy(), action=round(segment[5]), position=segment[6:8].copy(), alive=True), StateActionPair(radars=segment[8:13].copy(), action=round(segment[13]), position=segment[14:16].copy(), alive=True)]
...
def check_rules_flattened_one(segment, total_rules):
    ...
    rule_counter += evaluate(RULE_ORDER[0], lambda: distance > 30)

    # Rule 2: Check if actions are not the same
    action1 = segment[5]
    action2 = segment[13]
    rule_counter += evaluate(RULE_ORDER[1], lambda: action1 != action2)

    # Rule 3: Check if left radar is greater than right radar
    left_radar = segment[8]
    right_radar = segment[12]
    rule_counter += evaluate(RULE_ORDER[2], lambda: left_radar > right_radar)
